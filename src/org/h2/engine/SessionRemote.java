...
public class SessionRemote extends SessionWithState implements DataHandler {
    ...
    private Transfer initTransfer(ConnectionInfo ci, String db, String server)
        ...
        //for(String key : keys ){
            System.out.println("SessionRemote : 132 : key :"+keys.length);
        //}
    ...
    private void connectServer(ConnectionInfo ci) {
        //System.out.println("SessionRemote : 379 : connectionInfo: "+ci.getURL());
        String name = ci.getName(); //例如: "//localhost:9092/mydb"
        if (name.startsWith("//")) {
            name = name.substring("//".length()); //变成"localhost:9092/mydb"
        }
        //System.out.println("SessionRemote : 384 : connectionInfo.name: "+ci.getName());
        ...
        //System.out.println("SessionRemote : 384 : databasename: "+databaseName);
        ...
        if (server.indexOf(',') >= 0) {
            //System.out.println("SessionRemote : 424 : server "+server);
    ...
    public synchronized CommandInterface prepareCommand(String sql, int fetchSize) {
        checkClosed();
        System.out.println("SessionRemote : 512 :"+sql );
